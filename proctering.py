...
import wget
from alerts import warning_msg
...
def procter(frame):
    global process_this_frame,face_locations,face_encodings,face_names,known_face_encodings,known_face_names
    ...
    if process_this_frame:
        ...
        for face_encoding in face_encodings:
            ...
            if len(face_names) == 2:
                ...
                warning_msg("Multiple Faces Detected , Clicking screenshot")
                click_screenshot("Cam")
            if len(face_names) == 1:
                if face_names[0] == "Unknown" :
                    #print("Unkown Face Detected")
                    warning_msg("Unknown Face Detected")
                    click_screenshot("Cam")
...
while(True):
    ...
    for i in range(nums[0]):
        ...
        if int(classes[0][i] == 67):
            ...
            warning_msg("Mobile Phone Detected")
    if count == 0:
        print('No person detected')
        warning_msg('No person detected')
    elif count > 1: 
        print('More than one person detected')
        warning_msg("More than one person detected")
